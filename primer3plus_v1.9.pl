...
#create csv file
my $final_output_filename  = "output/results_$fasta_name.csv";
if ($multiplex eq "true") {
    $final_output_filename  = "output/results_multi_$fasta_name.csv";
}
open (my $final_output_out, '>', "$final_output_filename") or die "Could not create $final_output_filename.";

#introduction lines with basic information
print $final_output_out "Primers brought to you by Team DyNAmic \n";
print $final_output_out "FASTA File: ,, $fasta_file\n\n";

#a list of the primers/amplimers produced by Primer3
print $final_output_out "Primers produced: \n";
for (my $i = 0; $i < @left_primers; $i++) {
    print $final_output_out "Primer Set $i: ,, $left_primers[$i],,, $right_primers[$i]\n";
    print $final_output_out "Positions: ,, $left_primers_pos[$i],,, $right_primers_pos[$i] \n";
    print $final_output_out "Amplimer:,, $amplimers[$i] \n" 
}

if ($human_filter eq "humanY") {
    print $final_output_out "\nHuman Filter: ,, YES, Results: , Performed,, Threshold:,, $threshold%\n";
    for (my $amplimer_count = 0; $amplimer_count < @amplimers; $amplimer_count++) {
        print $final_output_out "Primer Set $amplimer_count: ,,";
        ...
        open (my $human_out, '<', "$human_filter_filename") or die "Could not create $human_filter_filename.";
            ...
            while ($blast_query =~ /(.+?)(>.+?)(\|.+?\|)(.+?\,)/g) {
                ...
                if ($species_percent >= $threshold) {
                    print $final_output_out "$species_name,, $species_percent%,,";
                }
            }
        close $human_out;
        print $final_output_out "\n";
...
if ($hmrgd_filter eq "hmrgdY") {
    print $final_output_out "HMRGD BLAST: ,, YES, Results:, Performed,, Threshold:,, $threshold%";
    ...
    print $final_output_out "HMRGD BLAST: ,, NO, Results: , Not Performed";
}
print $final_output_out "\n\n";

if ($dimer_filter eq "localY") {
    print $final_output_out "Primer-Dimer Test: ,, YES, Results: , Performed,, Threshold:,, $threshold%\n";
    ...
    open (my $dimer_out, '<', "$dimer_blast_filename") or die "Could not create $dimer_blast_filename.";
        ...
        print $final_output_out "Left Primers: \n";
        for (my $a = 0; $a < @left_primers; $a++) {
            $dimer_query =~ /($left_primers[$a]: )(.+?)(\|)/g;
            print $final_output_out "$left_primers[$a],,, $2 \n";
        }
         
        print $final_output_out "Right Primers: \n";
        for (my $b = 0; $b < @right_primers; $b++) {
            $dimer_query =~ /($right_primers[$b]: )(.+?)(\|)/;
            print $final_output_out "$right_primers[$b],,, $2 \n";
        }
        
} else {
    print $final_output_out "Primer-Dimer Test: ,, NO, Results: , Not Performed\n";
}

close $final_output_out;
